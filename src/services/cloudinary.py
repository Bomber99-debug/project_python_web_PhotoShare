...
def transformation_type(data: TransformRequest) -> str:
    parts: list[str] = []
    if data.width is not None: parts.append(f"w{data.width}")
    if data.height is not None: parts.append(f"h{data.height}")
    if data.crop is not None: parts.append(f"c_{data.crop.value}")
    if data.angle is not None: parts.append(f"a_{data.angle}")
    if data.effect is not None: parts.append(f"e_{data.effect}")
    if data.format is not None: parts.append(f"f_{data.format.value}")
    return "_".join(parts)
...
async def upload_image_bytes(data: bytes, *, folder: str, public_id: str) -> str:
    ...
    try:
        result = await asyncio.to_thread(
            cloudinary.uploader.upload, data, resource_type="image", folder=folder, public_id=public_id, overwrite=True
        )
    except Exception as exc:
