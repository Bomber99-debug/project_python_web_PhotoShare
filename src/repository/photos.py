...
async def create_photo(db: AsyncSession, *, user_id: int, description: str | None, image_url: str, public_id: str) -> Photo:
...
async def create_transform(
    db: AsyncSession, *, photo_id: int, transformation_type: str, transformed_url: str, qr_code_url: str | None
) -> PhotoTransform:
    item = PhotoTransform(
        photo_id=photo_id,
...
async def get_transform(db: AsyncSession, transform_id: int) -> PhotoTransform | None:
    return (await db.execute(select(PhotoTransform).where(PhotoTransform.id == transform_id))).scalar_one_or_none()


async def list_transforms(db: AsyncSession, photo_id: int) -> list[PhotoTransform]:
    stmt = select(PhotoTransform).where(PhotoTransform.photo_id == photo_id).order_by(PhotoTransform.created_at.asc())
    return list((await db.execute(stmt)).scalars().all())


async def search_photos(
    db: AsyncSession, *, keyword: str | None = None, tag: str | None = None, min_rating: float | None = None,
    sort_by: str = "date", order: str = "desc", user_id: int | None = None,
    date_from: datetime | None = None, date_to: datetime | None = None,
) -> list[Photo]:
    stats = (
        select(Rating.photo_id.label("photo_id"), func.avg(Rating.value).label("avg_rating"))
