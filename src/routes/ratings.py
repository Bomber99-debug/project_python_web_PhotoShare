...
from src.repository.ratings import create_rating, delete_rating, get_average, get_rating, get_user_photo_rating, list_photo_ratings
...
async def rate_photo(
    photo_id: int, data: RatingCreate, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_active_user)
):
...
async def rating_details(
    photo_id: int, db: AsyncSession = Depends(get_db), _: User = Depends(require_moderator_or_admin)
):
...
async def remove_rating(
    rating_id: int, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_active_user)
):
