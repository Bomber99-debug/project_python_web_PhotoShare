...
from src.repository.photos import (
    create_photo, delete_photo, get_or_create_tags, get_photo_by_id, get_photo_detail, set_photo_tags, update_photo
)
...
from src.services.cloudinary import delete_photo as delete_cloudinary_photo
from src.services.cloudinary import upload_photo
...
def build_detail(photo: Photo) -> PhotoDetailResponse:
    ...
    return PhotoDetailResponse(
        **base.model_dump(), comments=photo.comments, rating_summary=summary, transformed_photos=photo.transformed_photos
    )


@router.post("", response_model=PhotoResponse, status_code=status.HTTP_201_CREATED)
async def upload_user_photo(
    file: UploadFile = File(...), description: str | None = Form(default=None), tags: list[str] | None = Form(default=None),
    ...
    try:
        ...
        photo = await create_photo(
            db, user_id=user.id, description=description, image_url=uploaded["image_url"], public_id=uploaded["public_id"]
        )
        if normalized_tags:
...
async def update_user_photo(
    photo_id: int, data: PhotoUpdate, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_active_user)
) -> Photo:
...
async def delete_user_photo(
    photo_id: int, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_active_user)
) -> dict[str, str]:
    photo = await get_photo_by_id(db, photo_id)
    if photo is None:
        raise HTTPException(status_code=404, detail="Photo not found")
