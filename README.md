...
PhotoShare API — REST API для зберігання та керування фотографіями, користувачами, тегами, коментарями, рейтингами й
трансформаціями зображень.

Застосунок побудований на FastAPI, використовує PostgreSQL через асинхронний SQLAlchemy, JWT для автентифікації та
Cloudinary для зберігання й трансформації зображень.

API підтримує систему ролей, керування доступом, блокування користувачів, коментарі, рейтинги, пошук і фільтрацію
фотографій, генерацію QR-кодів для трансформованих зображень.
...
Каскадне видалення використовується для пов'язаних записів фотографій, коментарів, рейтингів та інших залежних
сутностей.
