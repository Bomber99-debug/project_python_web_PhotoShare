...
from src.repository.blacklist import (
    add_to_blacklist,
    delete_expired,
    is_blacklisted,
)
from src.schemas.photo_transform import (
    CropMode,
    ImageFormat,
    TransformRequest,
)
from src.services import cloudinary as cloudinary_service
from src.services import qr as qr_service
...
def test_cloudinary_auto_format_is_not_added_to_options():
    data = TransformRequest(
        width=200,
...
def test_cloudinary_configuration_requires_credentials(
    monkeypatch,
):
    monkeypatch.setattr(
        cloudinary_service.settings,
        "cloudinary_name",
        "",
    )
    monkeypatch.setattr(
...
def test_transformed_url(
    ...
    result = cloudinary_service.transformed_url(
        "photo-1",
        TransformRequest(
            width=300,
        ),
...
async def test_upload_photo_service(
    monkeypatch,
):
    monkeypatch.setattr(
        cloudinary_service,
...
async def test_upload_empty_photo(
    monkeypatch,
):
    monkeypatch.setattr(
        cloudinary_service,
...
async def test_delete_cloudinary_photo(
    monkeypatch,
):
    monkeypatch.setattr(
        cloudinary_service,
...
async def test_delete_cloudinary_photo_failure(
    monkeypatch,
):
    monkeypatch.setattr(
        cloudinary_service,
...
async def test_upload_generated_image(
    ...
    def fake_upload(
        ...
        return {
            "secure_url": "https://example.com/qr.png",
        }
    ...
    result = await cloudinary_service.upload_image_bytes(
        b"png-data",
...
async def test_upload_generated_image_failure(
    monkeypatch,
):
    monkeypatch.setattr(
        cloudinary_service,
...
def test_create_qr_image():
    result = qr_service._create_qr_image(
...
async def test_generate_qr_preserves_http_exception(
    monkeypatch,
):
...
async def test_generate_qr_generic_failure(
    monkeypatch,
):
...
async def test_blacklist_repository(
    db_session,
):
    now = datetime.now(timezone.utc)

    assert await is_blacklisted(
        db_session,
        "missing-token",
    ) is False
